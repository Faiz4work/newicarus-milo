...
# This is indeed's usa scraper
class IndeedSpider(scrapy.Spider):
    name = 'indeed'
    # allowed_domains = ['indeed.com/worldwide']
    # start_urls = ['http://indeed.com/worldwide/']
    days = '1'
    start_urls = ['https://www.indeed.com/find-jobs.jsp']
    ...
    def parse(self, response):
        links = response.xpath('//*[@id="letter_selector"]//td[@class="col_b"]/a/@href').getall()
        links = [response.urljoin(i) for i in links]

        for link in links:
            yield Request(url=link, callback=self.parse_title_page)
